...
## 6. `/annotation4`（輪郭タイル方式）の試作
- サーバー側でポリゴン輪郭をSVGタイルとして合成し、座標ローデータを配布せずに描画できるUIを構築。
- `/api/annotation4/hit` でヒットテスト＋オーバーレイ生成、`/overlay` でキュー情報のみのハイライト更新に対応。
- フロントエンドはタイル画像のブレンドで `/annotation` に近い操作感を再現し、座標JSONを保持しない構成を確認。
