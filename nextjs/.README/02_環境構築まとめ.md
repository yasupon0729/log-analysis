...
## 🖥️ Electron（ローカルGUI）環境

Docker は従来通りサーバー起動に使い、Electron は **ホストOS上で起動**して `http://localhost:3729` を表示します。
デフォルトは **Electron無効** です。
...
### 2. Electron モードの有効化（任意）
`/home/yasunari/KNiT/log-analysis/.env.electron` を作成/編集し、`ELECTRON_MODE=1` にします。
無効化したい場合は `0` に戻すか、ファイルを削除します。
...
### 3. 起動
`make up` で Docker を起動した後、`ELECTRON_MODE=1` の場合のみ Electron が自動で起動します。
...
### 4. 文字化け対策（日本語/絵文字）
Linux/WSL 環境ではフォント不足で □ が出る場合があります。
その場合は以下を入れてから Electron を再起動します。
...
### 補足
- Electron は Docker コンテナ内では動かさず、**ホストOS側で実行**します。
- `.env.electron` が無い場合は Electron を起動しません。
...
JudgeImage で利用可能なユーザー ID は `src/lib/users/config.ts` の定数で管理しています。将来的に認証を導入するまでは、ここを編集して利用者を追加してください。
...
3. **systemd ユーザーサービスで自動起動を設定**
   ...
   - `StrictHostKeyChecking=accept-new` により初回接続時にホスト鍵を自動登録します。
...
{
  "scripts": {
    "dev": "next dev --turbopack --port 3729",
    "build": "NODE_OPTIONS=\"--require ./scripts/patch-fs-rename.js\" node node_modules/next/dist/bin/next build",
...
※ WSL などで `EXDEV` が発生する場合に備えて、ビルド時は `scripts/patch-fs-rename.js` を読み込みます。
